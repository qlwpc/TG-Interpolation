...
def main():
    ...
    snapshot = HERE / "v1_snapshot_kernels.py"
    spec = importlib.util.spec_from_file_location("_tgnomask_v1_snapshot", snapshot)
    v1 = importlib.util.module_from_spec(spec)
    sys.modules[spec.name] = v1
    spec.loader.exec_module(v1)
    path=HERE/f"typed_benchmark_{os.environ.get('SLURM_JOB_ID')}.json"
    result={"status":"running","groups":[],"torch":torch.__version__,
            "gpu":torch.cuda.get_device_name(),"job":os.environ.get("SLURM_JOB_ID"),
            "protocol":"B4 H12 N2048 D64 BF16; production layout; 10 groups from 4451; 5 rounds, 20 eval / 10 train iterations; randomized order; 5 warmups. Compare exact v1 snapshot and fused reordering; typed times include all layout work and backward.",
            "v1_snapshot_sha256":hashlib.sha256(snapshot.read_bytes()).hexdigest(),
    ...
    methods=("causal_flash","tgnomask_sdpa","tgnomask_flex","tgnomask_typed_v1","tgnomask_typed")
    try:
        for index,old in enumerate(groups):
            ...
            def forward(method):
                ...
                if method=="tgnomask_typed_v1":return v1.typed_attention(q,k,v,layout)
            ...
            for workload,iterations in (("eval",20),("train",10)):
                with torch.set_grad_enabled(workload=="train"):
                    ...
                    samples={m:[] for m in methods};order_rng=random.Random(6198+index+1000*(workload=="train"))
