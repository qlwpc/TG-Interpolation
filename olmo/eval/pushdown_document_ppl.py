...
def score_pushdown_native_candidates(
    ...
    *,
    ...
    if use_cache:
        ...
        prefix_logits = (
            F.linear(prefix_last, model.transformer.wte.weight)
            if model.config.weight_tying else model.transformer.ff_out(prefix_last)
        )
...
def evaluate_pushdown_document_ppl(
    ...
    *,
    ...
    if min(eval_batch_size, max_batch_tokens, max_batch_attention_elements, max_sequence_length) <= 0:
    ...
    for index, (doc_id, original) in enumerate(corpus):
        ...
        if score_fn is score_pushdown_native_candidates:
            ...
            active_cache = prefix_cache if use_kv_cache and prefix_cache is not None and prefix_cache.context == context else None
            if use_kv_cache and context and active_cache is None:
                cache_rebuilds += 1
