...
workers=${2:-8}
native_data=${3:-dataset/bbc-news/testppl/native_model_topk_300_v2}
output_dir=${4:-docppl_runs/${model}_native_full}
python_bin=${PYTHON_BIN:-python}
if [[ "$model" != gpst && "$model" != pushdown ]]; then
  echo "model must be gpst or pushdown" >&2; exit 2
fi
if [[ ! "$workers" =~ ^[1-9][0-9]*$ ]]; then
  echo "workers must be a positive integer" >&2; exit 2
fi
read -r documents samples_per_sentence < <(
  "$python_bin" -c 'import json,sys; m=json.load(open(sys.argv[1])); print(m["document_count"], m["candidate_slots"])' "$native_data/manifest.json"
)
if [[ ! "$documents" =~ ^[1-9][0-9]*$ || "$workers" -gt "$documents" ]]; then
  echo "workers cannot exceed the positive corpus document count" >&2; exit 2
fi
...
gpst_max_batch_attention_elements=${GPST_MAX_BATCH_ATTENTION_ELEMENTS:-16777216}
mkdir -p "$output_dir"
...
if [[ -n "${DOCUMENT_BOUNDS:-}" ]]; then
  ...
  previous=-1
  for bound in "${document_bounds[@]}"; do
    if [[ ! "$bound" =~ ^(0|[1-9][0-9]*)$ ]] || (( bound <= previous || bound > documents )); then
      echo "DOCUMENT_BOUNDS must be strictly increasing integers within the corpus" >&2; exit 2
    fi
    previous=$bound
  done
  if (( document_bounds[0] != 0 || previous != documents )); then
    echo "DOCUMENT_BOUNDS must cover all documents from 0 through $documents" >&2; exit 2
  fi
fi
...
for ((worker=0; worker<workers; worker++)); do
  ...
  if [[ "$model" == gpst ]]; then
    CUDA_VISIBLE_DEVICES=$gpu PYTHONPATH=. \
      ...
      --checkpoint "${GPST_CHECKPOINT:-saved_models/gpst-bbc-unsup/model.bin}" --native-data "$native_data" \
      --start-document "$start" --end-document "$end" --eval-batch-size 64 \
      --max-batch-actions 65536 --max-batch-attention-elements "$gpst_max_batch_attention_elements" --log-every 100 \
    ...
    OLMO_FLEX_ATTENTION_NUM_STAGES=1 CUDA_VISIBLE_DEVICES=$gpu PYTHONPATH=. \
      ...
      --checkpoint "${PUSHDOWN_CHECKPOINT:-saved_models/pushdown/step34354-unsharded}" --native-data "$native_data" \
      --start-document "$start" --end-document "$end" --eval-batch-size 64 \
      --max-batch-tokens 65536 --max-batch-attention-elements "$max_batch_attention_elements" --log-every 100 \
      --attachment-normalization "${PUSHDOWN_ATTACHMENT_NORMALIZATION:-stack_legal}" \
      --document-result-dir "$document_result_dir" --resume-document-results \
      >"$result" 2>"$log" &
    pids+=("$!")
  else
...
"$python_bin" scripts/merge_native_document_ppl.py "$model" "$document_result_dir" \
  --expected-documents "$documents" --expected-samples-per-sentence "$samples_per_sentence" \
