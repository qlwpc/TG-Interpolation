...
from olmo.gpst.model.r2d2_base import R2D2Base
from olmo.gpst.data_structure.py_backend import CPPChartTableManager
from olmo.gpst.model.fast_parser import TransformerParser
from olmo.gpst.model.tree_encoder import InsideEncoder, OutsideEncoder
import torch
from olmo.gpst.data_structure.tensor_cache import TensorCache, CacheType
from olmo.gpst.model.r2d2_common import SPECIAL_TOKEN_NUM
import torch.nn.functional as F
from olmo.gpst.model.weighted_sum_func import WeightedSumFunc
from olmo.gpst.utils.math_util import gumbel_softmax
...
class InsideOutsideModule(R2D2Base):
    ...
    def forward(self,
        ...
        # NOTE: these tensors are torch::from_blob views into the C++
        # TableManager's buffers (no ownership). On GPU, .to('cuda') copies into
        # owned memory; on CPU, .to('cpu') is a no-op and would return a dangling
        # view once the CPPChartTableManager is destroyed at forward() exit. We
        # therefore force an owning copy with .clone() so the returned tensors
        # stay valid on both CPU and GPU.
        ldr_cache_ids = ldr_cache_ids.to(self.device, non_blocking=True).clone()
        position_ids = position_ids.to(self.device, non_blocking=True).clone()
        tgt_ids = tgt_ids.to(self.device, non_blocking=True).clone()
        ext_ids = ext_ids.to(self.device, non_blocking=True).clone()
        token_indices = token_indices.to(self.device, non_blocking=True).clone()

        span_masks = span_masks.to(self.device, non_blocking=True).clone()
        split_targets = split_targets.to(self.device, non_blocking=True).clone()
