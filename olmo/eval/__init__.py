...
def build_downstream_evaluator(
    ...
    if is_unit_test:
        ...
        if eval_cfg.type in (EvaluatorType.tg_doc, EvaluatorType.terminal_doc):
            ...
            # prep_examples mutates doc_index. Prefer those exact boundaries;
            # retain a fallback for older datasets that expose only cumulative
            # doc_index after prep_examples.
            ...
            if document_ends is not None:
                ...
                if doc_idx is not None:
                    doc_sents = (
                        doc_idx.tolist()
                        if hasattr(doc_idx, "tolist")
                        else list(doc_idx)
                    )
                    group_starts = torch.LongTensor(
                        [0] + [int(s) * int(sent_size) for s in doc_sents]
                    )
